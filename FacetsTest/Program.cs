...
namespace FacetsTest
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var products = await productsQueries.GetProducts("Products/Options", null, new Dictionary<string, IList<string>>());
            Console.WriteLine("Number of products found: {0}", products.Count);
            var facets = await productsQueries.GetProductsFacets("Products/Options", null, new Dictionary<string, IList<string>>());
            PrintFacetResult(facets);

            Console.WriteLine("Results filtering by brand='Nike'");
            var productsByBrand = await productsQueries.GetProducts("Products/Options", null, new Dictionary<string, IList<string>> { { "Brand", new List<string> { "Nike" } } });
            Console.WriteLine("Number of products found: {0}", productsByBrand.Count);
            var facets2 = await productsQueries.GetProductsFacets("Products/Options", null, new Dictionary<string, IList<string>> { { "Brand", new List<string> { "Nike" } } });
            PrintFacetResult(facets2);

            Console.WriteLine("Results filtering by availability=true and Size='9'");
            var productsAvailable = await productsQueries.GetProducts("Products/Options", true, new Dictionary<string, IList<string>> { { "Size", new List<string> { "9" } } });
            Console.WriteLine("Number of products found: {0}", productsAvailable.Count);
            var facets3 = await productsQueries.GetProductsFacets("Products/Options", true, new Dictionary<string, IList<string>> { { "Size", new List<string> { "9" } } });
